...
tree = xml_read('tape1_part1.xgtf')
...
----------------object[:]
--------------------ATTRIBUTE
---------------------------framespan 'xxxx:xxxx'        char     (or 'xxx:xxxx yyy:yyyy zzz:zzzz')
---------------------------id:                          double
---------------------------
--------------------attribute[:]
--------------------------attribute(1)
...
for cnt_frame_total = 1:1:frame_total          % 第几�?
    ...
    for cnt_object = 1:1:object_length                 % object 
        ...
        object_framespan = object(cnt_object).ATTRIBUTE.framespan;
        
        object_framespan_split = regexp(object_framespan,' ','split');
        [object_framespan_split_length, object_framespan_split_length_temp] = size(object_framespan_split);
        for cnt_object_framespan_split = 1:1:object_framespan_split_length_temp
           object_frame_total = regexp(char(object_framespan_split(cnt_object_framespan_split)),':','split');
           object_frame_total_min = str2num(char(object_frame_total(1)));
           object_frame_total_max = str2num(char(object_frame_total(2)));
           
           if cnt_frame_total >= object_frame_total_min && cnt_frame_total <= object_frame_total_max
