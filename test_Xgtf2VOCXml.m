...
for cnt_frame_total = 1:1:5 %frame_total          % ?????????
    ...
    cnt_person_eachframe = 1;              % ?¡¥???¡ì?¡±?¨¦?¡é??-????oo??¡ã
    for cnt_object = 1:1:object_length                 % object ?????¡ã¨¦??
        object_name = object(cnt_object).ATTRIBUTE.name;
        object_id = object(cnt_object).ATTRIBUTE.id;
        if(strcmp(object_name,'PERSON'))            % ?|??????¡¥a??ooa??¡ã¡À??¡ì???
            object_attritube = object(cnt_object).attribute;    % ¨¨¡¥£¤object????¡À?????a???
            ...
            for cnt_object_attribute = 1:1:object_attritube_length 
                if(strcmp(object_attritube(cnt_object_attribute).ATTRIBUTE.name,'Location'))    % ?|??????¡¥a????????????¡¥a??¡ã¡À??¡ì??-

                    data_COLON_bbox = object_attritube(cnt_object_attribute).data_COLON_bbox;
                    [data_length,data_length_temp] = size(data_COLON_bbox);                     
 
                    for num = 1:1:data_length           % ¨¨¡¥£¤object?????¡ì??¡ã??¡ª??¡ã¨¦??
                        ...
                        for cnt_frame = frame_min:1:frame_max                           %¨¨¡¥£¤??¡ì??¡ã??¡ª????¡­¡¤??¡°??¡ã??¨¨????¡ä??????xxx --> xxx)
